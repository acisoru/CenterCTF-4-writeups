...
use cipher::{consts::U16, AlgorithmName, BlockCipher, InvalidLength, KeyInit, KeySizeUser, array::Array, typenum::Unsigned, BlockDecrypt,
    BlockEncrypt, BlockSizeUser};
...
pub fn encrypt_block(key: &[u8], pt: &[u8]) -> Vec<u8>{
    let mut state = <Serpent as KeyInit>::new_from_slice(key).unwrap();
    let mut block = Array::clone_from_slice(pt);
    state.encrypt_block(&mut block);
    block.as_slice().to_vec()
}
pub fn decrypt_block(key: &[u8], ct: &[u8]) -> Vec<u8>{
    let mut state = <Serpent as KeyInit>::new_from_slice(key).unwrap();
    let mut block = Array::clone_from_slice(ct);
    state.decrypt_block(&mut block);
    block.as_slice().to_vec()
}

pub fn encrypt_blocks(key: &[u8], pt: &[u8]) -> Vec<u8>{
    type Block = cipher::Block<Serpent>;
    let mut blocks = pt.chunks(16).map(|chunk|{Block::clone_from_slice(chunk)}).collect::<Vec<Block>>();
    // let block = Block::clone_from_slice(pt);
    let mut state = <Serpent as KeyInit>::new_from_slice(key).unwrap();
    state.encrypt_blocks(&mut blocks);
    blocks.concat()
}
pub fn decrypt_blocks(key: &[u8], ct: &[u8]) -> Vec<u8>{
    type Block = cipher::Block<Serpent>;
    let mut blocks = ct.chunks(16).map(|chunk|{Block::clone_from_slice(chunk)}).collect::<Vec<Block>>();
    // let block = Block::clone_from_slice(pt);
    let mut state = <Serpent as KeyInit>::new_from_slice(key).unwrap();
...
fn test_encrypt_one_block(){
    let key= b"aboba1337lolkek:";
    let pt = b"MORGENSTERN666**";

    let enc = encrypt_block(key,pt);
    println!("encrytped: {:?}", enc);
    let dec = decrypt_block(key,enc.as_slice());
...
fn test_encrypt_many_blocks(){
    let key= b"aboba1337lolkek:";
    let pt = b"MORGENSTERN666**MORGENSTERN666**";

    let enc = encrypt_blocks(key,pt);
    println!("enc_many: {:?}",enc);
    let dec = decrypt_blocks(key,enc.as_slice());
    println!("dec_many: {:?}",dec);

    assert_eq!(dec.as_slice(), pt);

    let enc_one_block =  encrypt_block(key, &pt[0..16]);
    assert_eq!(enc_one_block, enc[0..16]);
    assert_eq!(enc_one_block, enc[16..32]);
}
